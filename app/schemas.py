...
class Profile(StrictModel):
    ...
    budget_min: float | None = Field(default=None, ge=0, le=1e9)
    bathrooms_min: int = Field(default=0, ge=0, le=100)
    floor_min: int | None = Field(default=None, ge=0, le=200)
    exclude_occupied: bool = False
    metro_stations: list[str] = Field(default_factory=list, max_length=30)
    search_bounds: list[SearchBounds] = Field(default_factory=list, max_length=4)
    ...
    def valid_range(self):
        if self.budget_min is not None and self.budget_max is not None and self.budget_min > self.budget_max:
            raise ValueError("Orçamento mínimo não pode exceder o máximo")
