...
ALIASES = {
    ...
    'combined_monthly_cost': ('iptuPlusCondominium', 'complexFee'),
    'metro_minutes': ('walkingTimeToMetro',), 'image_url': ('coverImage',),
    'property_type': ('type',),
}
NUMBERS = ('price', 'area', 'bedrooms', 'bathrooms', 'parking', 'floor', 'condo_fee',
...
def _number(value, field):
    ...
    if field not in ('floor', 'latitude', 'longitude') and number < 0:
        raise ValueError(f'{field}: valor negativo')
    if field in ('bedrooms', 'bathrooms', 'parking', 'floor') and number != int(number):
