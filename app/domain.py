...
def evaluate_property(property, peers, profile, now=None):
    ...
    for field, preference, label, direction in checks:
        ...
        if limit is None or (direction == "min" and limit <= 0):
            continue
    ...
    for category, label, fields in (("price", "Orçamento", ["price", "monthly_cost"]), ("location", "Localização", ["city", "neighborhood", "metro_minutes"]), ("quality", "Características e qualidade", ["area", "bedrooms", "parking", "elevator"])):
