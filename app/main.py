...
from .schemas import AlertUpdate, Budget, CommitPreview, Login, Profile, Register, SourceCreate, SourceUpdate, Tracking
...
def sources(user: User):
    ...
    for name, url in PORTALS:
        if not any(s["name"] == name and s["kind"] == "feed" for s in items):
            items.append({"id": None, "name": name, "url": url, "kind": "portal", "status": "pending_access", "enabled": False, "authorized": False, "last_attempt": None, "last_success": None, "error": "Integração de catálogo não habilitada. Requer rota de acesso autorizada.", "record_count": 0})
...
def update_source(sid: int, payload: SourceUpdate, user: User):
    try:
        with storage.transaction() as conn:
            row = conn.execute("SELECT * FROM sources WHERE id=? AND user_id=? AND kind='feed'", (sid, user["id"])).fetchone()
            if not row:
...
def refresh_all(user: User):
    with closing(storage.connect()) as conn:
        rows = conn.execute("SELECT id,name FROM sources WHERE user_id=? AND kind='feed' AND enabled=1 AND authorized=1", (user["id"],)).fetchall()
    results = []
    for row in rows:
        try:
    ...
    return {"results": results, "message": "Atualizações concluídas" if results else "Nenhum feed autorizado está habilitado. Configure uma fonte para atualização diária."}
...
def status(user: User):
    with closing(storage.connect()) as conn:
        return {"schedule": schedule_info(conn), "legacy_import_available": os.environ.get("IMOVEL_ENABLE_LEGACY_IMPORT") == "1" and user["id"] == 1}
