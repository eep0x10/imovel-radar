...
def profile_for(conn, user_id):
    return json.loads(conn.execute("SELECT profile FROM users WHERE id=?", (user_id,)).fetchone()[0])
...
def ingest(conn, user_id, records, origin="import", source_id=None):
    ...
    for original in records:
        ...
        if previous == record:
            result["unchanged"] += 1
            continue
        if previous and {k: v for k, v in previous.items() if k != "observed_at"} == {k: v for k, v in record.items() if k != "observed_at"}:
...
def enrich_all(conn, user_id):
    ...
    for item in items:
        ...
        item["price_change"] = item["price"] - prices[-2] if len(prices) > 1 else None
...
def detail(conn, user_id, pid):
    ...
    item["price_change"] = item["price"] - item["history"][-2]["price"] if len(item["history"]) > 1 else None
...
def refresh_source(user_id, source_id):
    ...
    try:
        with storage.transaction() as conn:
            ...
            if not row or row["kind"] != "feed" or not row["enabled"] or not row["authorized"]:
        ...
        if parsed.get("errors"):
            raise ValueError("Feed contém registros inválidos; dados anteriores preservados. Corrija o arquivo e tente novamente.")
        ...
        records = [{**r, "external_id": f"feed:{source_id}:{r['external_id']}", "feed_name": source["name"],
            "observed_at": r.get("observed_at") or fetched_at} for r in parsed["records"]]
        with storage.transaction() as conn:
            result = ingest(conn, user_id, records, "feed", source_id)
            conn.execute("UPDATE sources SET status='healthy',last_success=?,error=NULL WHERE id=?", (storage.now_iso(), source_id))
