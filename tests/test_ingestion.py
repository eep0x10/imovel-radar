...
@pytest.mark.parametrize('value,expected', [(True, True), (False, False), (' true ', True), ('não', False), (0, False), (1, True), (None, None), ('unknown', None), ('uncertain', None)])
def test_occupied_preserved_without_guessing_unknown(value, expected):
    record = parse([{**BASE, 'occupied': value}])['records'][0]
    assert record['occupied'] is expected


def test_metro_station_canonical_and_legacy_alias():
    direct = parse([{**BASE, 'metro_station': '  Belém  '}])['records'][0]
    legacy = parse([{**BASE, 'nearestMetroStation': 'Belém'}])['records'][0]
    assert direct['metro_station'] == legacy['metro_station'] == 'Belém'
    assert direct['provenance']['metro_station']['source'] == BASE['source']
    assert parse([BASE])['records'][0]['metro_station'] is None
    assert parse([{**BASE, 'metro_station': {'name': 'Belém'}}])['errors']


def test_floor_interval_preserved_without_inventing_exact_floor():
    record = parse([{**BASE, 'floor_min_reported': 0, 'floor_max_reported': 3}])['records'][0]
    assert record['floor'] is None
    assert record['floor_min_reported'] == 0 and record['floor_max_reported'] == 3
    assert parse([{**BASE, 'floor_min_reported': 3, 'floor_max_reported': 0}])['errors']
    assert parse([{**BASE, 'floor_min_reported': 1.5}])['errors']
