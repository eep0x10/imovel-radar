...
from app.services import ingest, enrich_all, detail
...
def test_fresh_provenance_does_not_duplicate_history_and_metadata_preserves_price_drop(database):
    with storage.transaction() as conn:
        first = record(price=300000, observed_at='2026-09-17T12:00:00Z')
        drop = record(price=270000, observed_at='2026-09-18T12:00:00Z')
        sighting = record(price=270000, observed_at='2026-09-19T12:00:00Z')
        assert ingest(conn, 1, [first])['created'] == 1
        assert ingest(conn, 1, [drop])['updated'] == 1
        assert ingest(conn, 1, [sighting])['unchanged'] == 1
        assert conn.execute('SELECT COUNT(*) FROM observations').fetchone()[0] == 2
        current = enrich_all(conn, 1)[0]
        assert current['observed_at'] == sighting['observed_at']
        assert current['provenance']['price']['observed_at'] == sighting['observed_at']
        assert current['price_change'] == -30000
        changed = record(price=270000, observed_at='2026-09-20T12:00:00Z', bathrooms=2)
        assert ingest(conn, 1, [changed])['updated'] == 1
        current = enrich_all(conn, 1)[0]
        assert current['price_change'] == -30000
        assert detail(conn, 1, current['id'])['price_change'] == -30000
        rebound = record(price=280000, observed_at='2026-09-21T12:00:00Z', bathrooms=2)
        assert ingest(conn, 1, [rebound])['updated'] == 1
        assert enrich_all(conn, 1)[0]['price_change'] == 10000
        assert conn.execute("SELECT COUNT(*) FROM alerts WHERE kind='price_drop'").fetchone()[0] == 1
