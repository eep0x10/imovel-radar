...
export function priceChange(p) {
  const c = p.price_change_30d;
  if (!c || !Number.isFinite(c.amount) || c.amount === 0) return "";
  const down = c.amount < 0;
  return `<div class="price-change ${down ? "price-down" : "price-up"}"><strong>${down ? "↓ Preço caiu" : "↑ Preço subiu"} ${money(Math.abs(c.amount))}${c.percent == null ? "" : ` (${number(Math.abs(c.percent))}%)`}</strong><span>De ${money(c.previous_price)} para ${money(c.price)} · observado em ${esc(new Date(c.observed_at).toLocaleDateString("pt-BR"))} · últimos 30 dias</span></div>`;
}
...
export function card(p, compare) {
  ...
  return `<article class="property">${photo(p)}<div class="property-body"><div class="property-title"><div><span class="tag">${esc(p.source)} · ${esc(listingStatus(p.status))}</span><span class="tag">${p.construction_status === "under_construction" || p.construction_status === "off_plan" ? "Na planta / em construção" : "Pronto"}</span>${!p.observed_at ? '<span class="tag amber">Data não confirmada</span>' : ""}<h3>${esc(listingTitle(p))}</h3><p>${esc([p.address, p.neighborhood, p.city].filter(Boolean).join(" · "))}</p></div><button class="save ${p.saved ? "saved" : ""}" data-save="${p.id}" data-saved="${p.saved ? "true" : "false"}" aria-label="${p.saved ? "Remover dos" : "Adicionar aos"} salvos" aria-pressed="${!!p.saved}">${p.saved ? "♥" : "♡"}</button></div><div class="features"><span>${number(p.area)} m²</span><span>${number(p.bedrooms)} quartos</span><span>${number(p.parking)} vagas</span>${metroInfo(p)}</div><div class="price-line"><div class="price">${money(p.price)}<small>${p.combined_monthly_cost != null ? `Cond. + IPTU combinados: ${money(p.combined_monthly_cost)}${p.combined_cost_period === "unknown" ? " · período a confirmar" : "/mês"}` : `Condomínio: ${money(p.condo_fee)}`}</small></div><div class="opportunity">${opportunity(e)}<small>${e.comparables_count || 0} comparáveis · confiança ${confidence(e.confidence)}</small></div></div>${priceChange(p)}${e.pending_requirements?.length ? `<p class="small"><strong>Filtros a confirmar:</strong> ${esc(e.pending_requirements.join(" · "))}</p>` : ""}<div class="score-line"><span class="score">${score(e.fit_score)}</span> aderência pessoal · qualidade ${score(e.quality_score)}</div><div class="card-bottom"><span>${money(p.price / p.area)}/m²</span><label><input type="checkbox" data-compare="${p.id}" ${compare.has(p.id) ? "checked" : ""}> Comparar</label><div class="listing-actions">${link(p.url, `Ver anúncio · ${p.source || "Fonte original"}`) || '<span class="small">Link original não informado</span>'}<button class="text-button" data-detail="${p.id}">Ver dossiê</button></div></div></div></article>`;
}
export const stages = {
  saved: "Salvo",
