...
export async function api(path, options = {}) {
  ...
  const response = await fetch(`/api${path}`, { ...options, headers }).catch(
    () => {
      throw Error(
  ...
  if (requestToken && requestToken !== token)
    throw Error("Sessão alterada. Repita a solicitação na conta atual.");
...
export async function download(path, name) {
  ...
  if (requestToken !== token)
    throw Error("Sessão alterada. Solicite o arquivo novamente.");
  if (r.status === 401 && requestToken) {
    setToken(null);
    window.dispatchEvent(new Event("session-expired"));
  }
  if (!r.ok)
