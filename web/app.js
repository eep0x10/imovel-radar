...
function portalCatalog(r) {
  ...
  return `<section class="portal-catalog"><h2>Coletar anúncios dos portais</h2><p class="subtitle">A coleta usa sua busca salva. Ajuste orçamento, localização e características em <a href="#profile">Minha busca</a>.</p><div class="source-grid">${r.catalog
    .map((p) => {
      ...
      return `<article class="panel"><h3>${esc(p.name)}</h3><p>${link(p.url, "Abrir portal")}</p>${existing ? "<p>Fonte já configurada. Use os controles de coleta abaixo.</p>" : p.supported && p.portal ? `<button class="primary" data-portal="${esc(p.portal)}">Ativar e coletar anúncios</button>` : "<p>Coleta ainda não disponível. Você pode importar um arquivo desta fonte.</p>"}</article>`;
    })
    .join("")}</div></section>`;
}
      ...
      (b.onclick = () =>
        busy(b, async () => {
          ...
          for (const result of r.results || []) {
            if (result.source_id && result.result)
              state.sourceResults[result.source_id] = result.result;
          }
